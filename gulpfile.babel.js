...
const sass = require("gulp-sass")(require("sass"));// dart-sass 사용하는경우
...
function scss() {
  return gulp
    ...
    .pipe(sass({ outputStyle: "expanded" }).on("error", sass.logError)) // 사스로 컴파일 기본값 expanded, compressed  지원 나머지 지원 안함
    .pipe(autoprefixer())
